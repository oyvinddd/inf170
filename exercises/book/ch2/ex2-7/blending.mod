...
# The amount of each intermediate used to make 
# products must equal the amount available:
subject to Available {i in I}:
	sum {j in J} X[i,j] = a[i];

# The amount of a product made must equal the 
# sum of amounts of the components blended into it:
subject to Amount {j in J}:
	sum {i in I} X[i,j] = Y[j];

# For each product, the total attributes contributed 
# by all intermediates must not exceed the total allowed:
subject to Allowed {j in J,k in K}:
	sum {i in I} r[i,k] * X[i,j] <= u[j,k] * Y[j];